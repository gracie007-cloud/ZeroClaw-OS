# Development manual for ZeroClaw
This guide will show you how to setup a local development environment for ZeroClaw in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - ZeroClaw runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- ZeroClaw is available on GitHub [github.com/agent0ai/zeroclaw](https://github.com/agent0ai/zeroclaw).
- You can download the files using a browser and extract or run `git clone https://github.com/agent0ai/zeroclaw` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/agent0ai/zeroclaw`, so my project folder is `~/Desktop/zeroclaw`.
...
- ZeroClaw comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.

1. Open your IDE and open the project folder using `File > Open Folder` and select your folder, in my case `~/Desktop/zeroclaw`.
...
## Step 4: Run ZeroClaw in the IDE
Great work! Now you should be able to run ZeroClaw from your IDE including real-time debugging.
...
After inserting my API key in settings, my ZeroClaw instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of ZeroClaw in Docker
- Some parts of Nuvho require standardized linux environment, additional web services and preinstalled binaries that would be unneccessarily complex to set up in a local environment.
- To make development easier, we can use existing Nuvho instance in docker and forward some requests to be executed there using SSH and RFC (Remote Function Call).

1. Pull the docker image `agent0ai/zeroclaw` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
...
This is how it looks in my example: port `80` is mapped to `8880` on the host and `22` to `8822`, `/a0` folder mapped to `/Users/frdel/Desktop/zeroclaw`:
...
ZeroClaw runs code inside the container by default. If you are running the framework locally in your IDE but want tools (like code execution) to run in Docker, configure RFC in **Settings -> Development** and point it to a running ZeroClaw container. This routes execution through SSH/RFC to the container while keeping the UI and agent loop on your host.


# Congratulations!

You have successfully set up a complete ZeroClaw development environment! You now have:
...
- The ability to develop, debug, and test ZeroClaw features seamlessly

You're now ready to contribute to ZeroClaw, create custom extensions, or modify the framework to suit your needs. Happy coding!
...
For development and testing, you can override default settings using the `.env` file with `NUVHO_SET_` prefixed variables:

```env
# Add to your .env file
NUVHO_SET_chat_model_provider=ollama
NUVHO_SET_chat_model_name=llama3.2
NUVHO_SET_chat_model_api_base=http://localhost:11434
NUVHO_SET_memory_recall_interval=5
```
...
- Navigate to your project root in the terminal and run `docker build -f DockerfileLocal -t zeroclaw-local --build-arg CACHE_DATE=$(date +%Y-%m-%d:%H:%M:%S) .`
